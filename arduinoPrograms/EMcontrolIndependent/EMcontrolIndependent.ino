...
void loop() {
  ...
  if (TinyWireS.available()){           // got I2C input!
    ...
    if(byteCommand == 0x21) {            //analog write from i2c read
      analogWrite(EMleft, TinyWireS.receive());
    else if(byteCommand == 0x22) {            //analog write from i2c read
          analogWrite(EMright, TinyWireS.receive());
